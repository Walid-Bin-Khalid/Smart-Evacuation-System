...
class PathfindingService {
  ...
  //  │ Ground   │ corridor   │ C3         │ G_C3                 │
  //  │ Ground   │ stairs     │ STAIRS     │ G_STAIRS             │
  //  │ First    │ room       │ R5         │ F1_R5_C              │
  //  │ First    │ corridor   │ C7         │ F1_C7                │
  //  │ Second   │ stairs     │ STAIRS     │ F2_STAIRS            │
...
//   String? resolveNodeId(EvacueeLocation location) {
//     final prefix = _floorPrefix(location.floor);
//     final roomNo = location.roomNumber.trim().toUpperCase();
//     final targetType = _nodeType(location.areaType);

//     // ── Step 1: Build candidate ID and try exact match ──
//     String candidateId;
//     if (targetType == 'room_center') {
//       // Room nodes follow pattern: {prefix}_{roomNo}_C
//       candidateId = '${prefix}_${roomNo}_C';
//     } else {
//       // Corridor/stairs/door: {prefix}_{roomNo}
//       candidateId = '${prefix}_$roomNo';
//     }
...
//     //   • floor matches
//     //   • node type matches
//     //   • node ID contains the roomNumber string
//     final fuzzyMatches = graph.nodes.where((node) {
//       final floorMatch =
//           node.floor.toLowerCase() == location.floor.trim().toLowerCase();
//       final typeMatch = node.type == targetType;
//       final idContainsRoom = node.id.toUpperCase().contains(roomNo);
//       return floorMatch && typeMatch && idContainsRoom;
//     }).toList();

//     if (fuzzyMatches.isNotEmpty) {
//       // If multiple fuzzy matches, prefer the one whose ID most closely
//       // resembles the candidate (shortest ID = least ambiguous)
//       fuzzyMatches.sort((a, b) => a.id.length.compareTo(b.id.length));
...
//   // ─────────────────────────────────────────────
//   EvacuationPath? findBestEvacuationPath(String startNodeId) {
//     final exits = graph.nodes.where((n) => n.isExit).toList();
//     if (exits.isEmpty) return null;
