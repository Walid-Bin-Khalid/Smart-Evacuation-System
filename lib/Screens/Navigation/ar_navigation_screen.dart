...
import '../../Services/websocket_service.dart'; // ← CHANGED
...
class _ARNavigationScreenState extends State<ARNavigationScreen> {
  ...
  StreamSubscription<EvacuationAlert?>?
  _alertSub; // ← EvacuationAlert from websocket_service
  StreamSubscription<String>? _floorSub;
  StreamSubscription<int>? _nodeReachedSub;
  ...
  Future<void> _loadGraph() async {
    try {
      ...
      // ← CHANGED: WebSocketService
      final alert = WebSocketService().currentAlert;
  ...
  void _listenToLiveAlerts() {
    // ← CHANGED: WebSocketService
    _alertSub = WebSocketService().alertStream.listen((alert) {
      if (!mounted) return;
